...
```{r}
raincloud <- function(sector_input="residential", start_year=2011, end_year=2019, y_axis_max=65){
  d <- long_price %>%
...
output <-
  ...
  geom_violinhalf(aes(x=acep_energy_region, 
                      y=price, 
                      fill=acep_energy_region),
                  position = position_nudge(x=.1, y=0)) +
  
  geom_jitter(aes(x=acep_energy_region, 
                  y=price, 
                  color=acep_energy_region), # Use color instead of fill for points
              alpha=0.75, 
              width = .07,
              height = 0) +
  ...
  ylim(c(0,y_axis_max)) + #take axis limits as arguments?
  coord_flip() 
...
```{r warning=FALSE, eval=knitr::is_latex_output()}
raincloud(sector_input="commercial", start_year=2011, end_year=2019, y_axis_max=120)
```
...
sample textsample textsaple textsample text
sample textsample textsaple textsample textsample textsample textsaple textsample textsample textsample textsaple textsample textsample textsample textsaple textsample text
...
```{r warning=FALSE, eval=knitr::is_latex_output()}
raincloud(sector_input="other", start_year=2011, end_year=2019, y_axis_max=120)
```
