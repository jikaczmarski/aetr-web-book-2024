...
viewof capacity_input = Inputs.select(new Map([["Capacity Metadata", capacity_wide_metadata], 
...
button(net_generation_wide, `net_generation_wide.csv`)
...
```{ojs}
prices_description = metadata.resources[4].description;
prices_raw = FileAttachment("data/working/prices/prices.csv").csv({typed: true});

prices = tidy(prices_raw,
            filter((d) => d.year < 2020))

prices_path = metadata.resources[4].path;
prices_metadata = metadata.resources[4].schema.fields;
...
button(prices_metadata, `prices_metadata.csv`);
...
```{ojs}
weighted_prices_description = metadata.resources[5].description;
...
weighted_prices_path = metadata.resources[5].path;
weighted_prices_metadata = metadata.resources[5].schema.fields;
...
button(weighted_prices_metadata, `weighted_prices_metadata.csv`);
