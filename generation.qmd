...
```{ojs}
//| echo: false
import {tidy, groupBy, rename, summarize, sum, mutate, select, n, nDistinct, mean, filter, pivotWider, pivotLonger, leftJoin, slice, fullJoin} from "@pbeshai/tidyjs"
...
While generation did not change significantly during the observed time period, the resources from which generation occurred, did. Figure X shows how the mix of generation fuels changed between 2011 and 2021.[^2]
...
Plot.plot({
  ...
  caption: "Figure note: Storage is not included as its net generation is negative.",
  insetLeft: 0,
  insetRight: 0,
  ...
  marks: [
    Plot.areaY(
      join_shares.filter((d) => d.acep_region === "Coastal"),
      {
        ...
        y:"generation_share", 
        fill: "fuel_type", 
        tip: {format: {x: "d", y: (d) => `${(d).toLocaleString(undefined, {maximumFractionDigits: 0})}%`}}
      }
    ),
    Plot.ruleY([0])
  ],
